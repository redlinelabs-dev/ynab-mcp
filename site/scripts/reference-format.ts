// Pure Markdown-rendering helpers behind generate-reference.ts, split out so they're
// importable by vitest without touching the filesystem or the compiled `dist/` output —
// see CLAUDE.md and issue #28.
...
export function propDescription(raw: unknown): string {
...
export function paramsTable(tool: ToolDef): string {
...
export function milliunitsNote(tool: ToolDef): string | null {
...
export function renderTool(tool: ToolDef): string {
...
export function renderGroupPage(group: ToolGroup, tools: ToolDef[], preamble: string): string {
  const sortedTools = [...tools].sort((a, b) => a.name.localeCompare(b.name));
  const title = `Reference: ${GROUP_TITLES[group]}`;
  const description = GROUP_DESCRIPTIONS[group];
  const generatedBody = sortedTools.map(renderTool).join("\n\n");
  return [
    "---",
    `title: "${title}"`,
